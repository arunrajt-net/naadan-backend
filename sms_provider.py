...
class MSG91OTPService:
    ...
    def send_otp(phone: str, event_type: str = "OTP_SIGNUP") -> bool:
        ...
        try:
            ...
            msg_content = f"[MSG91 SendOTP] Verification code generated by MSG91 and sent to {phone}"
            if res.status_code == 200 and res_data.get("type") == "success":
                log_sms_event(phone, event_type, "MSG91", msg_content, res_data.get("request_id"), "SENT")
                return True
            else:
                log_sms_event(phone, event_type, "MSG91", msg_content, None, "FAILED", error_msg=str(res_data))
                return False
        except Exception as e:
            print(f"[MSG91 SendOTP ERROR] {e}")
            log_sms_event(phone, event_type, "MSG91", "[MSG91 SendOTP] Exception", None, "FAILED", error_msg=str(e))
            return False
