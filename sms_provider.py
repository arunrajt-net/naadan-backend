...
class MSG91OTPService:
    @staticmethod
    def send_otp(phone: str, event_type: str = "OTP_SIGNUP") -> bool:
        auth_key = os.environ.get("MSG91_AUTH_KEY", "").strip()
        if not auth_key:
            print("[MSG91] Auth key missing for SendOTP.")
            return False
        ...
        if not template_id:
            print(f"[MSG91] Template ID missing for event: {event_type}")
            
        # Call MSG91 Send OTP API (passing params in query string as required by MSG91 v5 API)
        ...
        try:
            ...
            res_data = res.json()
            print(f"[MSG91 SendOTP RESPONSE] {res_data}")
            
            # Log to SMS Audit Logs for admin stats
            msg_content = f"[MSG91 SendOTP] Verification code generated by MSG91 and sent to {phone} using template {template_id}"
            if res.status_code == 200 and res_data.get("type") == "success":
                log_sms_event(phone, event_type, "MSG91", msg_content, res_data.get("request_id"), "SENT", template_id)
                return True
            else:
                log_sms_event(phone, event_type, "MSG91", msg_content, None, "FAILED", template_id, error_msg=str(res_data))
                return False
        except Exception as e:
            print(f"[MSG91 SendOTP ERROR] {e}")
            log_sms_event(phone, event_type, "MSG91", "[MSG91 SendOTP] Exception", None, "FAILED", template_id, error_msg=str(e))
            return False
    ...
    def verify_otp(phone: str, otp: str) -> bool:
        auth_key = os.environ.get("MSG91_AUTH_KEY", "").strip()
        if not auth_key:
            print("[MSG91] Auth key missing for VerifyOTP.")
            return False
        ...
        try:
            ...
            res_data = res.json()
            print(f"[MSG91 VerifyOTP RESPONSE] {res_data}")
            if res.status_code == 200 and res_data.get("type") == "success" or res_data.get("message") == "OTP verified successfully":
                return True
            return False
        except Exception as e:
            print(f"[MSG91 VerifyOTP ERROR] {e}")
            return False
